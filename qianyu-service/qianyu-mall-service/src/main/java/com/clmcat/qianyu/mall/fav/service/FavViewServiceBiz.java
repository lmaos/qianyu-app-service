...
@Service
public class FavViewServiceBiz {

    @Resource
    private FavServiceBiz favServiceBiz;
    ...
    public FavActionResultVO addFav(long userId, FavTargetDTO dto) {
        FavStatus.FAV_TYPE_INVALID.assertThrowResEx(dto == null || dto.getType() == null || (dto.getType() != 1 && dto.getType() != 2));
        FavStatus.FAV_TARGET_NOT_FOUND.assertThrowResEx(FavConvert.isNullOrNonPositive(dto.getTargetId()));

        // 幂等处理：已收藏则直接返回
        FavFavorite existing = favServiceBiz.selectByUserAndTarget(userId, dto.getTargetId(), dto.getType());
        if (existing != null) {
    ...
    public FavActionResultVO cancelFav(long userId, FavTargetDTO dto) {
        FavStatus.FAV_TYPE_INVALID.assertThrowResEx(dto == null || dto.getType() == null);
    ...
    public FavStatusVO getFavStatus(long userId, FavTargetDTO dto) {
        if (dto == null || FavConvert.isNullOrNonPositive(dto.getTargetId()) || dto.getType() == null) {
    ...
    public FavBatchStatusResultVO getBatchFavStatus(long userId, FavBatchStatusDTO dto) {
        ...
        for (FavTargetDTO target : dto.getTargets()) {
            boolean isFav = favKeys.contains(target.getTargetId() + "_" + target.getType());
