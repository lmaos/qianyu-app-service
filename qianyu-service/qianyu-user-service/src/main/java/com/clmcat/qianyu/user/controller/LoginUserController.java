 ...
 * 用户登录相关接口。
 * <p>
 * 当前类使用 {@link ApiController} 暴露 HTTP API，包含手机号验证码发送、手机号、邮箱、社交登录、账号登录和签名验证能力。
 * <p>
 * OpenAPI 页面地址（默认本地启动）：
 ...
 */
@Tag(name = "用户登录接口", description = "提供手机号验证码发送、手机号、邮箱、社交、账号登录，以及 token 签名/验签接口。")
@ApiController
...
public class LoginUserController {
    ...
    @Resource
    PhoneVerifyCodeSendServiceBiz phoneVerifyCodeSendServiceBiz;


    /**
     * 发送手机号验证码。
     *
     * @param dto 参数说明：phone 为待接收验证码的手机号，secondVerifyCode 为预留的二次校验码
     * @return 发送结果
     */
    @Operation(summary = "发送手机号验证码", description = "参数说明：dto.phone 为待接收验证码的手机号；dto.secondVerifyCode 为预留扩展字段，当前不参与校验。")
    @PostMapping("/phone/verify_code")
    public PhoneVerifyVo sendPhoneVerifyCode(@Params(description = "手机号验证码发送参数") PhoneVerifyDto dto) {
        return phoneVerifyCodeSendServiceBiz.sendVerifyCode(dto);
    }
    ...
    @PostMapping("/email")
    public LoginResultVo email(@Params(description = "邮箱登录参数") EMailLoginDto dto) {
    ...
    @PostMapping("/social")
    public LoginResultVo social(@Params(description = "第三方社交登录参数") SocialLoginDto dto) {
    ...
    @PostMapping("/signer")
    public String signer(@Params(description = "签名参数，继承 TokenInfoDto") SignerDto dto) {
    ...
    public SignerDto verifier(@Params(name = "token", description = "待解析验证的登录 token 字符串") String token) {
